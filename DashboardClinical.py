...
def load_data_from_mysql():
    ...
    resultados_patientinfo = cursor.fetchall()
    cursor.execute("SELECT * FROM tumorcharacteristics")
    resultados_tumorcharacterist = cursor.fetchall()
    connection.close()

    df_patientinfo = pd.DataFrame(resultados_patientinfo, columns=["PatientID", "Date of Birth (Days)", "Race and Ethnicity", "Days to death (from the date of diagnosis)"])
    df_patientinfo["Age"] = abs(df_patientinfo["Date of Birth (Days)"]) // 365
    df_patientinfo.to_csv('Edad.csv', index=False)
...
def load_data_from_csv(filename):
    ...
    columnas_deseadas = ['Days to death', 'ER', 'PR', 'HER2']
    df = df[columnas_deseadas]
    df['Murio ER'] = df.apply(lambda row: '1' if row['Days to death'] != 'NP' and row['ER'] == 1
                              else '2' if row['Days to death'] != 'NP' and row['ER'] == 0
                              else '4' if row['Days to death'] == 'NP' and row['ER'] == 1
                              else '3', axis=1)
    df['Murio PR'] = df.apply(lambda row: '1' if row['Days to death'] != 'NP' and row['PR'] == 1
                              else '2' if row['Days to death'] != 'NP' and row['PR'] == 0
                              else '4' if row['Days to death'] == 'NP' and row['PR'] == 1
                              else '3', axis=1)
    df['Murio HER2'] = df.apply(lambda row: '1' if row['Days to death'] != 'NP' and row['HER2'] == 1
                                else '2' if row['Days to death'] != 'NP' and row['HER2'] == 0
                                else '4' if row['Days to death'] == 'NP' and row['HER2'] == 1
                                else '3', axis=1)
    return df
...
def update_line_chart(attr, old_range, new_range):
    ...
    filtered_data = df_patientinfo['Age'][(df_patientinfo['Age'] >= age_min) & (df_patientinfo['Age'] <= age_max)]
...
df_patientinfo = load_data_from_mysql()
...
x = df_patientinfo['Age'].value_counts().sort_index().index
y = df_patientinfo['Age'].value_counts().sort_index().values
...
age_slider = RangeSlider(start=df_patientinfo['Age'].min(), end=df_patientinfo['Age'].max(), value=(df_patientinfo['Age'].min(), df_patientinfo['Age'].max()), step=1, title="Filtrar por Edad")

# Definir la función de actualización de datos para el gráfico de líneas
update_data = CustomJS(args=dict(source=line_source, df_patientinfo=ColumnDataSource(df_patientinfo), age_slider=age_slider),
                       code="""
                            const data = df_patientinfo.data;
...
conteos = df_patientinfo['Race and Ethnicity'].value_counts()
...
df_tumor = load_data_from_csv('tumor.csv')

# Conteo de valores únicos en las columnas 'Murio ER', 'Murio PR' y 'Murio HER2'
murio_er_counts = df_tumor['Murio ER'].value_counts()
murio_pr_counts = df_tumor['Murio PR'].value_counts()
murio_her2_counts = df_tumor['Murio HER2'].value_counts()
